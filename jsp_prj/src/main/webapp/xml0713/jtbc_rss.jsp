...
function requestRss(url){
	...
	$.ajax({
		...
		error:function(xhr){
			alert("문제가 발생했습니다.");
			consol.log(xhr.status + " / " + xhr.statusText);
		},
		success:function(xmlDoc){
			...
			output += $(pubDateNode).text() + "<br>";
			
			// item node들을 찾아서 하위 자식 노드들을 반복시킨다.
			$(xmlDoc).find("channel > item").each(function(ind, itemNode){
			output += "<div><table class='table table-hover'>";
			output +="<tr>"
			output +="<td style='width: 80px;'>" + (ind+1) + "</td>";
			output +="<td style='width: 80px;'>제목</td>";
			output +="<td style='width: 350px;'><a href='"
			+ $(itemNode).find('link').text()+"'>"
			+ $(itemNode).find("title").text() 
			+"</a></td>";
			output +="<td style='width: 80px;'>작성일</td>";
			output +="<td style='width: 150px;'>" + $(itemNode).find("pubDate").text() + "</td>";
			output +="</tr>"
			
			output +="<tr>"
			output +="<td></td>"
			output +="<td  colspan='2'>" + $(itemNode).find("description").text() + "</td>";
			output +="</tr>"
...
<body>
	...
	<main>
		...
		<table class="table">
			...
			<td><a href="javascript:requestRss('${ item.rssLink }')">
			<%-- <td><a href="jtbc_rss.jsp?link=${ item.rssLink }"> --%>
			<c:out value="${ item.title }"/></a></td>
			</c:forEach>
			</tr>
		</table>
		</div>
